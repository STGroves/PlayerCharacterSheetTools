///<reference path="BlockNode.js"/>
///<reference path="../../ClassExtension.js"/>

class FunctionBlockNode extends BlockNode {
  #protObj = null;

  #inputFlags = null;

  get Inputs() { return this.#protObj.get("connections").slice(1); }
  ...
  constructor(title, returnType, connectionTypes, inputTypes, prot = {}) {
    let protObj = ClassExtension.enforceProtectedObject(new.target, FunctionBlockNode, prot);

    super(title, returnType, connectionTypes, prot);
    this.#protObj = protObj.set("onNodeCreated", (it, evt) => this.#onNodeCreated(it, evt))
                            ...
                            .set("calculateHeight", (it) => this.#calculateHeight())
  ...
  #createInputZones() {
    ...
    for (let i = 0; i < this.#inputFlags.length; i++) {
      ...
      PROT_GET("connections").push(new ConnectionPoint({
        mainID: this.ID,
        postfix: `#INPUT${i}`
      },
      INPUT_RECT,
  ...
  #updateNode() {
    ...
    const PROT_GET = this.#protObj.get;
    const STYLE = PROT_GET("style");
    const X = STYLE.leftOffset;

    let y = STYLE.headerHeight;

    for (let i = 0; i < this.#inputFlags.length; i++) {
      const INPUT = PROT_GET("connections")[i];

      if (INPUT.Node == null)
        continue;

      INPUT.Node.updatePosition(new Vector(X, y));

      y += INPUT.Rect.Height;

      if (i < (this.#inputFlags.length - 1))
        y += STYLE.midHeight;
    }
  ...
  #calculateMiddle(pathObj, offset, index) {
    ...
    pathObj = this.#protObj.get("calculateTeeth").next(pathObj.drawRadialCurve(NC + offset, -90),
                                                        offset,
  ...
  #drawMiddle(nodeData) {
    ...
    for (let i = 0; i < this.Inputs.length; i++) {
      nodeData.outer.appendPath(PATH_CONSTANTS.upperMiddle.outer)
                    .drawGlobalY(this.Inputs[i].Rect.Height - NC2)
                    .appendPath(PATH_CONSTANTS.lowerMiddle[i].outer);
      nodeData.inner.appendPath(PATH_CONSTANTS.upperMiddle.inner)
                    .drawGlobalY(this.Inputs[i].Rect.Height - NC2)
