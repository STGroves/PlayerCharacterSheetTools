class ContextMenu {
  ...
  get Parent() { return this.#parentElement == null ? null : this.#parentElement.Menu; }
  get Position() { return this.#position; }

  static set Scope(scope) { ContextMenu.#scope = scope; }
  ...
  constructor(title = null) {
    ...
    if (title != null)
      this.addElement(new CMLabel(title, CMLabel.Alignment.MIDDLE))
          .addElement(new CMSeparator());
  }
  ...
  setHoveredItem(item) {
    ...
    if (this.#hoveredItem != null)
      this.#hoveredItem.onHover();
  }

  #checkBoundary(pos, scopeRect, htmlDims) {
  ...
  limitPosition(pos, root = false) {
    ...
    let limited = Vector.clone(pos);

    const BR_BOUNDS = this.#checkBoundary(pos, SCOPE_RECT, HTML_DIMS);

    if (root) {
      if (!BR_BOUNDS.x)
        limited.x = SCOPE_RECT.left + SCOPE_RECT.width - HTML_RECT.width;
    
      if (!BR_BOUNDS.y)
        limited.y = SCOPE_RECT.top + SCOPE_RECT.height - HTML_RECT.height;
    ...
    const TL_BOUNDS = this.#checkBoundary(new Vector(LEFT_X, pos.y),
                                          SCOPE_RECT,
                                          HTML_DIMS);
  ...
  close() {
    if (this.#hoveredItem != null && this.#hoveredItem instanceof CMSubMenu)
      this.#hoveredItem.Target.close();
