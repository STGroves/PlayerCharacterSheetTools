...
export default class DraggableArea {
  ...
  #setupEvents(svg) {
    svg.addEventListener('mousemove', (event) => {
      if (
        !this.#handlePassThroughEvent(event, this.#selectedTarget, (evt, target) => {
    ...
    svg.addEventListener('mousedown', (event) => {
      this.#handlePassThroughEvent(event, this.#selectedTarget, (evt, target) =>
        this.#handleClick(evt, target, 'pt-mousedown')
      );
    });

    svg.addEventListener('mouseup', (event) => {
      this.#handlePassThroughEvent(event, this.#selectedTarget || this.#lastTarget, (evt, target) => {
        this.#handleClick(evt, target, 'pt-mouseup');

        if (target === this.#lastTarget) this.#lastTarget = null;
      });
    });
  ...
  #handleClick(evt, target, type) {
    if (this.#currentList.length === 0) this.#updatePassThroughList(evt, target.BaseVisualElement);
  ...
  #handlePassThroughEvent(evt, target, callback) {
    if (this.#updating || target == null) return false;
  ...
  static #isBehindOtherElement(element, getBaseElementsListCallback = document.elementsFromPoint) {
    ...
    const TOP_LEFT = getBaseElementsListCallback(LEFT, TOP)
      .filter((x) => (x.hasAttribute('data-ignoreElement') ? !x.getAttribute('data-ignoreElement') : true))
      .filter(
        (x) =>
          x === element ||
          (x.hasAttribute('data-transparentElement') ? !x.getAttribute('data-transparentElement') : true)
      )[0];
    ...
    return TOP_LEFT !== element && TOP_RIGHT !== element && BOTTOM_LEFT !== element && BOTTOM_RIGHT !== element;
  }

  #updatePassThroughList(evt, root) {
    this.#currentList = DraggableArea.#getDOMElementsUnderDraggableElement(evt.clientX, evt.clientY, root).filter(
      (elem) =>
        !DraggableArea.#isBehindOtherElement(elem, (x, y) => {
          return DraggableArea.#getDOMElementsUnderDraggableElement(x, y, root);
        })
    );
  }

  static #getDOMElementsUnderDraggableElement(x, y, elementRoot) {
  ...
  #update(evt, target) {
    ...
    if (this.#currentList[0] !== this.#dragArea) {
      const MOVE_EVENT = new PassThroughEvent('pt-mousemove', {
        bubbles: evt.bubbles,
  ...
  #drag(evt) {
    ...
    if (this.#selectedTarget.AllowPassThroughEvents) this.#update(evt, this.#selectedTarget);
  }
